...
class StoryManager:
    ...
    def create_default_party(self) -> List[CharacterProfile]:
        ...
        class_defaults = {
            DnDClass.BARBARIAN: {
                ...
                "background": "A tribal warrior seeking to prove their worth in civilized lands."
            },
            DnDClass.BARD: {
                "name": "Melody Brightstring",
                "personality": "Charismatic storyteller who loves attention",
                "background": "A traveling performer collecting stories and songs from across the realm."
            },
            DnDClass.CLERIC: {
                "name": "Brother Marcus Lightbringer",
                "personality": "Devout and compassionate healer",
                "background": "A devoted priest on a sacred mission to spread their deity's influence."
            },
            DnDClass.DRUID: {
                "name": "Willow Moonwhisper",
                "personality": "Wise but distrustful of civilization",
                "background": "A guardian of ancient forests who reluctantly ventures into towns."
            },
            DnDClass.FIGHTER: {
                "name": "Sir Gareth Steelhand",
                "personality": "Disciplined and honor-bound",
                "background": "A seasoned knight seeking to uphold justice and protect the innocent."
            },
            DnDClass.MONK: {
                "name": "Zen Shadowstep",
                "personality": "Contemplative and disciplined",
                "background": "A seeker of enlightenment who has left their monastery to learn from the world."
            },
            DnDClass.PALADIN: {
                "name": "Aurelia Dawnbreaker",
                "personality": "Righteous and inspiring",
                "background": "A holy warrior bound by sacred oaths to fight evil wherever it lurks."
            },
            DnDClass.RANGER: {
                "name": "Finn Swiftarrow",
                "personality": "Independent and observant",
                "background": "A skilled tracker who protects civilization's borders from monsters."
            },
            DnDClass.ROGUE: {
                "name": "Shadow Quickfingers",
                "personality": "Cautious and opportunistic",
                "background": "A reformed thief using their skills for the greater good (mostly)."
            },
            DnDClass.SORCERER: {
                "name": "Ember Starborn",
                "personality": "Emotional and intuitive",
                "background": "A young magic-wielder learning to control their inherited arcane power."
            },
            DnDClass.WARLOCK: {
                "name": "Raven Darkpact",
                "personality": "Mysterious and driven",
                "background": "Bound by an otherworldly pact, seeking to fulfill their patron's agenda."
            },
            DnDClass.WIZARD: {
                "name": "Elara Scrollkeeper",
                "personality": "Scholarly and methodical",
                "background": "A learned scholar fascinated by the mysteries of arcane magic."
            }
        }
    ...
    def analyze_story_file(self, filepath: str) -> Dict[str, Any]:
        ...
        for request in dc_requests:
            suggestions = self._get_dc_suggestions_for_request(request, content)
        ...
        return {
            ...
            'overall_consistency': self._calculate_overall_consistency(consultant_analyses)
        }
    ...
    def _extract_dc_requests(self, content: str) -> List[str]:
        ...
        dc_section_match = re.search(r'## DC Suggestions Needed\\s*\\n([^#]*)', content, re.MULTILINE)
    ...
    def _get_dc_suggestions_for_request(self, request: str, story_context: str) -> Dict[str, Any]:
    ...
    def _calculate_overall_consistency(self, analyses: Dict[str, Any]) -> Dict[str, Any]:
        """Calculate overall consistency metrics."""
        if not analyses:
            return {"score": 0, "rating": "No character actions to analyze"}
        
        total_score = sum(analysis['consistency_score'] for analysis in analyses.values())
        ...
        return {
            ...
            'summary': f"{len(analyses)} characters analyzed, {len(all_positives)} positive notes, {len(all_issues)} issues found"
        }
    ...
    def update_story_with_analysis(self, filepath: str, analysis: Dict[str, Any]):
        ...
        content = self._update_section(content, "Character Consultant Notes", consultant_notes)
        content = self._update_section(content, "Consistency Analysis", consistency_section)
        
        # Write back to file
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
        
        print(f"✅ Updated story file with consultant analysis: {os.path.basename(filepath)}")
        ...
        if analysis.get('dc_suggestions'):
            ...
            for request, suggestions in analysis['dc_suggestions'].items():
                ...
                for character, suggestion in suggestions.items():
                    notes.append(f"- {character}: DC {suggestion['suggested_dc']} ({suggestion['reasoning']})")
                    if suggestion.get('alternative_approaches'):
                        notes.append(f"  - Alternatives: {', '.join(suggestion['alternative_approaches'])}")
                notes.append("")
    ...
    def _generate_consistency_section(self, analysis: Dict[str, Any]) -> str:
        ...
        sections.append(f"**Summary:** {overall.get('summary', 'No analysis available')}")
        sections.append("")
        ...
        if analysis.get('consultant_analyses'):
            ...
            for character, char_analysis in analysis['consultant_analyses'].items():
                sections.append(f"**{character}** - {char_analysis.get('overall_rating', 'Unknown')}")
                sections.append(f"Score: {char_analysis.get('consistency_score', 0)}/1.0\\n")
    ...
    def suggest_character_reaction(self, character_name: str, situation: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
