...
validate_character_file = (
    getattr(_validator_module, "validate_character_file", None)
    if _validator_module
    else None
)
...
def load_character_consultant(
    ...
    Args:
        ...
        ai_client: Optional AI client for the consultant
        verbose: If True, perform validation and print validation results.
                 If False, skip validation entirely for faster loading.

    Returns None if validation fails (when verbose=True) or loading fails.
    """
    # Only validate if verbose=True (validation is expensive)
    if verbose and VALIDATOR_AVAILABLE and validate_character_file:
