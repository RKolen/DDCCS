...
| POST | `/character/skill-plan` | Derive the class + species/subspecies plan for the skills step: granted skills/tools + skill/tool choice groups, the class **equipment** A/B choices (items vs gold), and the **subclass** choice (level + options). The class portion comes from the `class` taxonomy (`class_grant` paragraphs) and falls back to the JSON template + rules wiki; `source` reports which was used |
