...
| POST | `/spells/lookup` | Resolve a spell's stat block (level, school, casting, range, components, duration, concentration, ritual, description) from `spell:{slug}` pages at `RAG_RULES_BASE_URL` |
...
| POST | `/arc-draft/propose` | Propose the story arc a campaign's played sessions add up to, from its stored session recaps (never the story bodies, which will not fit a local context). One model call; returns `{ draft: null }` rather than failing when nothing usable comes back. Needs an **instruct** model (`ARC_DRAFT_PROFILE`, default `creative`) for the same reason `/relations/suggest` does |
| POST | `/arc-draft/npcs` | Read the NPC cast a campaign's played sessions name, each with a one-line role and a `known` flag saying whether a character already carries that name. A separate call from `/arc-draft/propose`: asking one prompt to both write an arc and enumerate a cast gets a worse answer at both |
...
| POST | `/story/events` | Chunk a story body and extract 6-12 selectable visual moments (`title`, `one_line`, `excerpt`). Never sends the whole story in one prompt. Optional `roster` names help the model stay on people Drupal already knows. Needs an **instruct** model (`STORY_IMAGE_PROFILE`, default `creative`) for the same reason `/relations/suggest` does; the same client backs `/story/scene`'s shot analysis |
| POST | `/story/scene` | Analyse one picked excerpt (setting, action, mood, named people), then render a **768x512** DreamShaper scene. Up to two IPAdapter leads, then staggered ReActor face swaps for remaining likenesses, with `POST /free` between ComfyUI steps. ReActor is optional: if `COMFYUI_REACTOR_SWAP_MODEL` is unset the scene still ships, and `used_ipadapter` / `swapped_faces` report what actually ran. Timeout is `COMFYUI_SCENE_TIMEOUT` (default 1800s). Accepts `shot` (wide/full/medium/close) and `angle` (front/three_quarter/side/behind); anything but `behind` bans rear views in the negative, without which every render came back as backs walking away. Likeness needs `COMFYUI_SCENE_IPADAPTER_MODEL` (a **face** adapter); the full-image `COMFYUI_IPADAPTER_MODEL` is never used here because it redraws the reference portrait instead of composing a scene. Returns base64 PNG plus seed, prompt, alt |
...
| `story_image_routes.py` | `/story/events` and `/story/scene` |
| `comfyui_guard.py` | Shared ComfyUI 503 checks for portrait and scene routes |
