...
class ItemRegistry:
    ...
    def __init__(
        self, registry_path: str = "game_data/items/custom_items_registry.json"
    ):
...
if __name__ == "__main__":
    ...
    print(
        "Official D&D items are NOT registered - they're looked up directly on wikidot.\n"
    )
    ...
    print("=" * 60)
    print(
        f"Can lookup 'Greataxe' (official): {registry.can_lookup_on_wiki('Greataxe')}"
    )
    print(
        f"Can lookup '{custom_magic.name}' (custom): "
        f"{registry.can_lookup_on_wiki(custom_magic.name)}"
    )
    print(
        f"Can lookup 'Longsword' (official): {registry.can_lookup_on_wiki('Longsword')}"
    )
