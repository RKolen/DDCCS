...
class StoryAnalyzer:
    ...
    def analyze_story_file(self, story_file_path: str) -> Dict[str, Dict[str, List[str]]]:
        """
    ...
    def _suggest_npc_creation(self, story_content: str, for_character: str) -> List[str]:
        ...
        for pattern in unnamed_patterns:
            ...
            for match in matches:
                role = match.group(1)
                if role not in [npc.lower() for npc in self.existing_npcs]:
                    suggestions.append(f"SUGGESTION: Create NPC '{role.title()}' - appears to interact with {for_character}")
        ...
        if len(context_around_dialogue) > 0:
            suggestions.append(f"SUGGESTION: Dialogue found without clear speaker - consider creating NPCs for {len(context_around_dialogue)} unnamed speakers")
        
        return suggestions
    
    def generate_npc_template(self, npc_name: str, role: str, interacted_with: str) -> Dict:
    ...
    def update_character_file(self, character_name: str, updates: Dict) -> bool:
        ...
        try:
            ...
            if 'major_plot_actions' in updates:
                ...
                character_data['major_plot_actions'].extend(updates['major_plot_actions'])
            ...
            if 'personality_traits' in updates:
                ...
                character_data['personality']['traits'].extend(updates['personality_traits'])
