...
Supports two modes:
1. Natural language combat prompts (RECOMMENDED) - Takes tactical descriptions
   and converts to narrative
...
class CombatNarrator:
    ...
    def narrate_combat_from_prompt(
        ...
        # Create AI prompt
        system_prompt = f"""You are an expert D&D combat narrator. Convert \
...
- Make it flow like a story, not a combat log"""
    ...
    def _parse_round(self, round_text: str, round_number: int) -> Dict[str, Any]:
        ...
        patterns = {
            "attack": (
                r"(\w+)\s+attacks\s+(\w+)\s+.*?rolls?\s+(\d+).*?"
                r"(?:hits?|misses?).*?(?:for\s+(\d+)\s+damage)?"
            ),
            "spell": (
                r"(\w+)\s+casts\s+(\w+)\s+.*?"
                r"(?:for\s+(\d+)\s+damage|healing\s+(\d+))?"
            ),
        ...
        for action_type, pattern in patterns.items():
            ...
            for match in matches:
                ...
                round_data["participants"].update(
                    [name for name in match if isinstance(name, str) and name.isalpha()]
                )

        return round_data

    def _create_action_data(
        self, action_type: str, match_data: tuple
    ) -> Dict[str, Any]:
        ...
        if action_type == "attack":
            action.update(
                {
                    ...
                    "damage": (
                        int(match_data[3])
                        if len(match_data) > 3 and match_data[3]
                        else 0
                    ),
                }
            )
            ...
            action.update(
                {
                    ...
                    "damage": (
                        int(match_data[2])
                        if len(match_data) > 2 and match_data[2]
                        else 0
                    ),
                    "healing": (
                        int(match_data[3])
            ...
            action.update({"target": match_data[0], "amount": int(match_data[1])})
        elif action_type == "healing":
            action.update({"target": match_data[0], "amount": int(match_data[1])})
        elif action_type == "status":
            action.update({"target": match_data[0], "effect": match_data[1]})
    ...
    def convert_to_narrative(self, fgu_text: str, style: str = "detailed") -> str:
        ...
        if not combat_data["rounds"]:
            return (
                "The combat summary could not be parsed. Please check the FGU format."
            )
    ...
    def _create_round_narrative(self, round_data: Dict[str, Any], style: str) -> str:
        ...
        for character, char_actions in character_actions.items():
            char_narrative = self._create_character_action_narrative(
                character, char_actions, style
            )
            narrative += f"\\n{char_narrative}"
    ...
    def _create_character_action_narrative(
        self, character: str, actions: List[Dict[str, Any]], style: str
    ) -> str:
    ...
    def _describe_attack(
        ...
        if damage > 0:
            ...
            if roll >= 20:
                return (
                    f"{attack_desc} with devastating precision, dealing {damage_desc}!"
                )
    ...
    def _create_combat_conclusion(self, combat_data: Dict[str, Any]) -> str:
        ...
        conclusion = "\\n**Combat Conclusion:**\\n"
        conclusion += (
            f"The battle lasted {round_count} round{'s' if round_count != 1 else ''}, "
        )
            ...
            if character in self.consultants:
                ...
                for action in actions:
                    if any(
                        ...
                        reaction = consultant.suggest_reaction(
                            f"combat situation: {action}"
                        )

                        # Check if action aligns with character nature
                        if "class_reaction" in reaction:
                            consistency_notes.append(
                                f"**{character}:** {reaction['class_reaction']}"
                            )
