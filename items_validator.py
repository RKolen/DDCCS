...
def validate_item_entry(item_key: str, item_data: Dict[str, Any]) -> List[str]:
    ...
    for field, expected_type in required_fields.items():
        if field not in item_data:
            ...
            errors.append(f"Item '{item_key}': Field '{field}' must be of type {expected_type.__name__}, got {type(item_data[field]).__name__}")
    
    # Validate item_type is one of the expected values
    if 'item_type' in item_data:
        valid_types = ['magic_item', 'weapon', 'armor', 'gear', 'tool', 'consumable', 'treasure']
        if item_data['item_type'] not in valid_types:
            errors.append(f"Item '{item_key}': item_type must be one of {valid_types}, got '{item_data['item_type']}'")
    ...
    if 'properties' in item_data and isinstance(item_data['properties'], dict):
        ...
        for prop_key, prop_value in item_data['properties'].items():
            if not isinstance(prop_value, (str, int, float, bool)):
                errors.append(f"Item '{item_key}': Property '{prop_key}' must be a string, number, or boolean")
    
    return errors


def validate_items_json(data: Dict[str, Any], filepath: str = "") -> Tuple[bool, List[str]]:
    """
    ...
    for item_key, item_data in item_entries.items():
        ...
        if any(c in name for c in disallowed_chars):
            errors.append(f"Item '{item_key}': Strange characters are not allowed in item name. Please use another name (disallowed: {''.join(disallowed_chars)}). Name: '{name}'")
...
def validate_items_file(filepath: str) -> Tuple[bool, List[str]]:
    ...
    # Validate the data
    return validate_items_json(data, filepath)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        filepath = sys.argv[1]
        is_valid, errors = validate_items_file(filepath)
        ...
        print_validation_report(items_file, is_valid, errors)
