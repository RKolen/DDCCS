...
class DDConsultantCLI:
    ...
    def list_characters(self):
        ...
        for i, name in enumerate(characters, 1):
            ...
            if profile:
                print(f"{i}. {name} ({profile.character_class.value} Level {profile.level})")
    ...
    def edit_character_profile(self, profile: CharacterProfile):
        ...
        while True:
            ...
            if choice == '1':
                ...
                new_motivations = input("Enter motivations (comma-separated): ").strip()
                if new_motivations:
                    profile.motivations = [m.strip() for m in new_motivations.split(',')]
            elif choice == '4':
                new_goals = input("Enter goals (comma-separated): ").strip()
                if new_goals:
                    profile.goals = [g.strip() for g in new_goals.split(',')]
            elif choice == '5':
                new_fears = input("Enter fears/weaknesses (comma-separated): ").strip()
                if new_fears:
                    profile.fears_weaknesses = [f.strip() for f in new_fears.split(',')]
            elif choice == '6':
                new_speech = input("Enter speech patterns (comma-separated): ").strip()
                if new_speech:
                    profile.speech_patterns = [s.strip() for s in new_speech.split(',')]
            elif choice == '7':
                new_style = input("Enter decision making style: ").strip()
                if new_style:
                    profile.decision_making_style = new_style
            elif choice == '8':
                self.story_manager.save_character_profile(profile)
                print(f"✅ Saved {profile.name}'s profile!")
                break
            elif choice == '0':
    ...
    def manage_story_series(self):
        ...
        try:
            ...
            if choice == 0:
                return
            elif 1 <= choice <= len(story_series):
    ...
    def create_new_story_series(self):
        ...
        try:
            filepath = self.story_manager.create_new_story_series(series_name, first_story_name, description)
    ...
    def create_story_in_series(self, series_name: str):
        ...
        try:
            filepath = self.story_manager.create_story_in_series(series_name, story_name, description)
    ...
    def view_story_details_in_series(self, series_name: str, stories: List[str]):
        ...
        try:
            ...
            if 1 <= choice <= len(stories):
                story_file = stories[choice - 1]
                story_path = os.path.join(self.story_manager.stories_path, series_name, story_file)
    ...
    def get_character_consultation(self):
        ...
        try:
            ...
            if 0 <= choice < len(characters):
                ...
                if situation:
                    reaction = self.story_manager.suggest_character_reaction(character_name, situation)
        ...
        if analysis.get('dc_suggestions'):
            ...
            for request, suggestions in analysis['dc_suggestions'].items():
                ...
                for character, suggestion in suggestions.items():
                    print(f"  {character}: DC {suggestion['suggested_dc']} ({suggestion['reasoning']})")
    ...
    def display_dc_suggestion(self, character: str, action: str, suggestion: Dict[str, Any]):
    ...
    def get_dm_narrative_suggestions(self):
        ...
        if characters:
            ...
            char_input = input("Enter character names present (comma-separated, or 'all', or leave blank): ").strip()
            if char_input.lower() == 'all':
                characters_present = characters
            elif char_input:
                characters_present = [name.strip() for name in char_input.split(',') if name.strip() in characters]
        
        # Let user select which NPCs are present
        npcs_present = []
        if npcs:
            print(f"\\nAvailable NPCs: {', '.join(npcs)}")
            npc_input = input("Enter NPC names present (comma-separated, or leave blank): ").strip()
            if npc_input:
                npcs_present = [name.strip() for name in npc_input.split(',') if name.strip() in npcs]
        
        # Get narrative suggestions
        suggestions = self.dm_consultant.suggest_narrative(prompt, characters_present, npcs_present)
...
def main():
    ...
    parser.add_argument("--command", help="Command to run", choices=['interactive', 'analyze', 'create-party'])
